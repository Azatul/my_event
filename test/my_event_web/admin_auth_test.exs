defmodule MyEventWeb.AdminAuthTest do
  use MyEventWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias MyEvent.Office
  alias MyEventWeb.AdminAuth
  import MyEvent.OfficeFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MyEventWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert Office.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = Office.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Office.get_admin_by_session_token(admin_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "admins_sessions:abcdef-token"
      MyEventWeb.Endpoint.subscribe(live_socket_id)
