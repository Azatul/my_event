defmodule MyEventWeb.AdminRegistrationLive do
  use MyEventWeb, :live_view

  alias MyEvent.Office
  alias MyEvent.Office.Admin
  ...
  def mount(_params, _session, socket) do
    changeset = Office.change_admin_registration(%Admin{})
  ...
  def handle_event("save", %{"admin" => admin_params}, socket) do
    case Office.register_admin(admin_params) do
      {:ok, admin} ->
        {:ok, _} =
          Office.deliver_admin_confirmation_instructions(
            admin,
  ...
  def handle_event("validate", %{"admin" => admin_params}, socket) do
    changeset = Office.change_admin_registration(%Admin{}, admin_params)
